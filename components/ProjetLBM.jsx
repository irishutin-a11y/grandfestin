...
function ProjetLBMPage() {
  ...
  return (
    <div className="pageProjet pageProjet--les-beaux-mets" ref={rootRef} data-screen-label={"Projet — " + p.shortTitle}>
        ...
        <div className="wrap proj-stats2">
          ...
          <div className="proj-stats2__grid reveal" ref={statsGridRef}>
            {p.stats.map((s, i) => {
              ...
              return (
                <div key={i} className={"proj-stat2 proj-stat2--" + ['teal', 'gold', 'terracotta', 'deep'][i % 4]}>
            ...
            <div className="proj-news">
              {featured3.map((a, i) => (
                <a key={i} className="proj-news__card reveal" href={a.href} target="_blank" rel="noopener noreferrer">
                  ...
                  <span className="proj-news__date">{plbmMonth(a.date)}</span>
                </a>
              ))}
            </div>
            {presseAlso.length > 0 && (
      ...
      {/* GALERIE */}
      <section className="proj-gallery" aria-label="Galerie photo Les Beaux Mets">
